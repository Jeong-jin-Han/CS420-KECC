...
impl<'s> JoinTable<'s> {
    ...
    pub(crate) fn lookup(
        ...
        let ret = loop {
            ...
            if end_values.contains_key(&(aid, bid)) || self.joins.contains(&(aid, bid)) {
                break bid;
            }
    ...
    fn optimize(&mut self, code: &mut FunctionDefinition) -> bool {
                ...
                match instr.deref() {
                    ...
                    Instruction::Load { ptr } => {
                        ...
                        if let RegisterId::Local { aid } = rid {
                            ...
                            let bid_join = join_table.lookup(*aid, *bid, &end_values.clone());
        ...
        while let Some((aid, bid)) = phinode_stack.pop() {
            ...
            for (bid_prev, _) in prevs {
                let bid_prev_join = join_table.lookup(aid, bid_prev, &end_values.clone());
