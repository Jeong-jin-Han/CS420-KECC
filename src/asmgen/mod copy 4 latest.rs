        ...
        for (name, decl) in &source.decls {
            match decl {
                ...
                ir::Declaration::Variable { dtype, initializer } => {
                    ...
                    // .word, .quad 등 선
                    let data_directives: Vec<asm::Directive> = match (&dtype, initializer) {
                        // ✅ 배열 + 초기화 리스트
                        (
                            ir::Dtype::Array { inner, size },
                            Some(ast::Initializer::List(init_list)),
                        ) => {
                            let mut directives = vec![];

                            for init in init_list {
                                if let ast::Initializer::Expression(expr) =
                                    &init.node.initializer.node
                                {
                                    if let Ok(constant) = ir::Constant::try_from(&expr.node) {
                                        match constant {
                                            ir::Constant::Int {
                                                value, width: 32, ..
                                            } => {
                                                directives.push(asm::Directive::Word(value as u32));
                                            }
                                            ir::Constant::Int {
                                                value, width: 64, ..
                                            } => {
                                                directives.push(asm::Directive::Quad(value as u64));
                                            }
                                            _ => {
                                                panic!("Unsupported array constant: {:?}", constant)
                                            }
                                        }
                                    }
                                }
                            ...
                            if padding > 0 {
                                directives.push(asm::Directive::Zero(padding as usize));
                            }
                        ...
                        (_, _) => {
                            let init_value: u64 = match init_value_opt {
                                Some(ir::Constant::Int { value, .. }) => value as u64,
                                Some(ir::Constant::Float { value, width }) => match width {
                                    32 => {
                                        let f: f32 = value.0 as f32;
                                        f.to_bits() as u64
                                    }
...
impl Asmgen {
    ...
    fn translate_function_prelogue(
        ...
        if call_flag {
            ...
            // sw s2, 16(sp) // this is for the allocation
            asm_block_instrs.push(asm::Instruction::SType {
                instr: asm::SType::Store(asm::DataSize::Double),
                rs1: asm::Register::Sp,
                rs2: asm::Register::S2,
        ...
        */
        ...
        for (lid, (local_rid, dtype, ptr_indicator_offset)) in
            ptr_indicator_offsets.into_iter().enumerate()
        {
            ...
            println!("ptr_dtype {} offset {}", ptr_dtype, offset);
            println!("===========================");
        ...
        for (i, dtype) in block.phinodes.iter().enumerate() {
            ...
            if !matches!(dtype.deref(), ir::Dtype::Struct { .. }) {
                let slot_off = self.stack_allocator.allocate_stack_slot(
                    &rid,
                    dtype.deref(),
                    &self.struct_list,
                );
                ...
                let slot_off =
                    self.stack_allocator
                        .allocate_stack_slot(&rid, &ptr_dtype, &self.struct_list);

                let flattend = flatten_struct_fields(dtype, &self.struct_list);
                // from  T1 <- caller_offset + S1

                // make the new pointer for the phinodes? T0 <- Sp + get_dtype() -> slot_off

                // base pointer load T0,
                // sp + slot_off 를 저장하는
                ...
                asm_block_instrs.push(asm::Instruction::IType {
                    ...
                    rd: asm::Register::T0,
                    rs1: asm::Register::Sp,
                    imm: asm::Immediate::Value((slot_off + 8) as u64),
                });
                ...
                // flatten된 모든 필드들을 복사
                let flattened = flatten_struct_fields(dtype, &self.struct_list);
            ...
            ir::BlockExit::Return { value } => {
                if !matches!(value.dtype(), ir::Dtype::Struct { .. }) {
                    ...
                    */
                    ...
                    base pointer load A0 S1 - (ret_slot)
                    source pointer load T0 SP + (value offset)
                    ...
                    */
                    ...
                    if let Some((value_rid, value_dtype)) = value.get_register() {
                        ...
                        let ret_offset = get_dtype_size(&value.dtype(), &self.struct_list);
                        let mut ret_slot = ret_offset;
                        add_padding(&mut ret_slot);
                        ...
                        asm_block_instrs.push(asm::Instruction::IType {
                            ...
                            rd: asm::Register::A0, // dst ptr
                            rs1: asm::Register::S1,
                            imm: asm::Immediate::Value((-ret_slot) as u64),
                        });
        ...
        match instruction.deref() {
                ...
                match *op {
                    ...
                    ast::BinaryOperator::Equals => {
                        if asm::Register::T1 == t1_reg {
                            ...
                            // feq.s or feq.d 사용
                            asm_block_instrs.push(asm::Instruction::RType {
                                instr: asm::RType::feq(lhs.dtype()),
                                rd: asm::Register::T0, // 결과는 int이므로 정수 레지스터에
                                rs1: t1_reg,
                                rs2: Some(t2_reg),
                            });
                    ...
                    ast::BinaryOperator::Less => {
                        if asm::Register::T1 == t1_reg {
                            ...
                            asm_block_instrs.push(asm::Instruction::RType {
                                ...
                                rd: asm::Register::T0, // float 비교 결과는 정수 레지스터에 저장
                                rs1: t1_reg,
                                rs2: Some(t2_reg),
                            });
                            ...
                            asm_block_instrs.push(asm::Instruction::SType {
                                instr: asm::SType::store(dtype.clone()),
                                rs1: asm::Register::Sp,
                                rs2: asm::Register::T0, // 비교 결과는 t0에 있음
                                imm: asm::Immediate::Value(offset as u64),
                            });
                        }
                    }
            ...
            ir::Instruction::Load { ptr } => {
                ...
                if !matches!(ptr_inner_dtype, ir::Dtype::Struct { .. }) {
                    ...
                    //     &self.struct_list,
                    // );
                    let dst_offset = self.stack_allocator.next_stack_offset;
                    self.stack_allocator.next_stack_offset +=
                        get_dtype_size(&ptr_inner_dtype, &self.struct_list);
            ...
            ir::Instruction::Call {
                ...
                let has_struct_arg = args
                    .iter()
                    .any(|arg| matches!(arg.dtype(), ir::Dtype::Struct { .. }));
                ...
                if !has_struct_arg {
                    ...
                    let ptr_ret = ir::Dtype::pointer(ret.deref().clone());
                    ret_offset = self.stack_allocator.allocate_stack_slot(
                        &dst_rid,
                        &ptr_ret,
                        &self.struct_list,
                    );
                ...
                for (i, arg) in args.iter().enumerate() {
                    if !matches!(arg.dtype(), ir::Dtype::Struct { .. }) {
                        ...
                        if i < 8 {
                            ...
                            caller_offset += get_dtype_size(&arg.dtype(), &self.struct_list);
                        }
                    } else {
                        // struct의 경우에는 ptr 저장하고 해당 ptr의 가리키는 영역
                        // 그리고 flatten 한다음에

                        /*
                        여기서는 그냥 현재 arg를 가리키는 pointer를 주면 될 것 같음
                        */
                        if let Some((rid, dtype)) = arg.get_register() {
                            let source_offset = self.stack_allocator.allocate_stack_slot(
                                rid,
                                dtype,
                                &self.struct_list,
                            );
                            ...
                            asm_block_instrs.push(asm::Instruction::IType {
                                ...
                                rd: asm::Register::T0,
                                rs1: asm::Register::Sp,
                                imm: asm::Immediate::Value((ret_offset + 8) as u64),
                            });
                            ...
                            self.stack_allocator.next_stack_offset +=
                                get_dtype_size(dtype, &self.struct_list);
                            ...
                            asm_block_instrs.push(asm::Instruction::IType {
                                ...
                                rd: asm::Register::T1, // src ptr
                                rs1: asm::Register::S2,
                                imm: asm::Immediate::Value(source_offset as u64),
                            });

                            // flatten된 모든 필드들을 복사
                            let dtype = arg.dtype();
                            let flattened = flatten_struct_fields(&dtype, &self.struct_list);
                            for (field_dtype, offset) in flattened.clone() {
                                let data_size = asm::DataSize::from_dtype(field_dtype);
